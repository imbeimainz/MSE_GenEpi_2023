...
* Clone or download the workshop repo, available at https://github.com/imbeimainz/RNAseqFrankfurt2023
...
of known gene sets, we can also apply a functional enrichment test. 
...
In each case, the gene sets that are tested for enrichment are obtained from the [Gene Ontology](http://geneontology.org/) 'biological process' catalog. 

```{r run-functionalenrichment}
library(GeneTonic)
...
# Bonus: Differential transcript expression with swish
...
```{r swishdte, message = FALSE, eval=eval_dte}
library(fishpond)
...
# Bonus: Differential transcript usage
...
```{r plotswishdexseq, eval=eval_dtu}
